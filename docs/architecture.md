...
| 限制 | 现状 | 适用边界 |
| --- | --- | --- |
| CSV 读取方式 | 面板每次请求重新读取整个文件，告警也按文件重算 | 适合 1-10 Hz 的监测；更高频率应换成数据库或时序库 |
| 数据保留 | CSV 只追加，没有自动滚动 | 长时间运行前应先归档 `data/telemetry.csv` |
| 服务暴露范围 | 面板只监听 `127.0.0.1`，没有登录与鉴权 | 定位是本机工具，不要直接暴露到公网 |
| 告警判定 | 单点越限即触发，没有去抖或持续时长条件 | 接真实设备时建议加"连续 N 次越限"再告警 |
| MQTT | 只生成 JSON 消息文本，未实现 MQTT 客户端与 broker 连接 | 需要上行时再接入传输层 |
